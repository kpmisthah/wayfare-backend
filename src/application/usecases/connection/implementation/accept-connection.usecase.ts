...
export class AcceptConnectionUseCase implements IAcceptConnection{
  ...
  async execute(id: string) {
    ...
    let updated = await this._connectionRepo.update(id,connectionUpdate);
    let conversation = await this._conversationUsecase.execute(connection.senderId,connection.receiverId);
