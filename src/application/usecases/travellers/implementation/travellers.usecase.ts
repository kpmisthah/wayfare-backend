...
export class TravellersUsecase implements ITravellersUsecase{
    ...
    async fetchTravellers(destination:string,userId:string):Promise<TravellersDto[]>{
        const tripPlan = await this._tripRepo.findTravellersByDestination(destination,userId)
        const users = await Promise.all(tripPlan.map((trip)=>this._userRepo.findById(trip.userId)))
        const userProfile = await Promise.all(users.map((user)=>user && this._profileRepo.findByUserId(user.id)))
        return TravellersMapper.toTravellersDto(users,tripPlan,userProfile)
    }

    async fetchTravellersByUserDestinations(userId:string):Promise<TravellersDto[]>{
        const destinations = await this._tripRepo.findDestinationByUserId(userId)
        if (!destinations.length) return [];
        const trips = await this._tripRepo.findTravellersByDestinations(destinations,userId)
