...
export class AdminRevenueRepository implements IAdminRevenueRepository{
    ...
    async getAllRevenue(){
        let totalStripeTransaction = await this._prisma.transaction.aggregate({
            _sum:{
                amount:true
            },
            where:{
        ...
        let totalWalletTransaction = await this._prisma.walletTransaction.aggregate({
            _sum:{
                amount:true
            },
            where:{
        ...
        let totalRevenue = (totalStripeTransaction._sum.amount || 0) + (totalWalletTransaction._sum.amount || 0)
        return totalRevenue
    }

    async getAllCommission(){
        let totalCommission = await this._prisma.booking.aggregate({
            _sum:{
    ...
    async getWalletBalance(){
        let walleBalance = await this._prisma.wallet.aggregate({
            _sum:{
                balance:true
            },
            where:{
    ...
    async activeAgenciesCount(){
        let agencyCount = await this._prisma.agency.count({
    ...
    async getTransactionSummary():Promise<WalletTransactionEntity[]>{
        let transaction = await this._prisma.walletTransaction.findMany({
            where:{
