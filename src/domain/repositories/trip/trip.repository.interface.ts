...
export interface ITripRepository extends IBaseRepository<AiTripEntity> {
  ...
  findTravellersByDestination(destination:string,userId:string):Promise<AiTripEntity[]>
  findDestinationByUserId(userId:string):Promise<string[]>
  findTravellersByDestinations(destinations:string[],userId:string):Promise<AiTripEntity[]>
}
